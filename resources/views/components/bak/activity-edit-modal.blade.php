<x-modal-form id="editIncomeModal{{ $activity->id }}" title="Edit penerimaan" name="income-edit-{{ $activity->id }}"
    method="post" action="/activity">
    ...
    <x-modal-form-select name="category_id" label="Kategori" placeholder="Pilih kategori" help="Kategori penerimaan"
        :options="$categories->where('activity_type', 'income')->pluck('name', 'id')" />
